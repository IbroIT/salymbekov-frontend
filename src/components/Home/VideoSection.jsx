...
const VideoSection = () => {
  ...
  const { content, mediaByKey } = usePageContent('home-video', {
    data: {
      platform: t('home.video.platform', 'YouTube'),
      youtube_id: 'SdluvCyzd6M',
    },
  });
  const data = content?.data || {};
  const videoUrl = mediaByKey.main_video?.url || data.video_url;
  const embedUrl = data.embed_url || (data.youtube_id ? `https://www.youtube.com/embed/${data.youtube_id}` : videoUrl);
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl overflow-hidden transform hover:scale-105 transition-transform duration-500">
            ...
            <div className="relative z-10 p-2">
              <div className="aspect-video rounded-2xl overflow-hidden bg-gray-900">
                <iframe
                  src={embedUrl}
                  title={content?.title || 'University Video'}
            ...
            <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm rounded-full px-4 py-2 shadow-lg">
              <div className="flex items-center gap-2 text-sm font-medium text-gray-700">
                ...
                {data.platform || t('home.video.platform', 'YouTube')}
              </div>
            </div>
          </div>
        </div>
